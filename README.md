...
Note: This tool is intended for educational and testing purposes only. Use it exclusively on cards you own or have explicit permission to validate.
...
bash
git clone https://github.com/your-username/mrcardchecker-gui.git
cd mrcardchecker-gui
2. Install Dependencies
Using APT (recommended for system stability):

bash
sudo apt update
sudo apt install python3-pyqt5 python3-requests
Or using PIP (latest versions):
...
bash
chmod +x main.py
4. (Optional) Install the .desktop File – Add to Kali Menu
bash
sudo cp mrcardchecker.desktop /usr/share/applications/
sudo update-desktop-database
Now you can launch the tool from Applications → Penetration Testing → MrCardChecker.
...
bash
python3 main.py
Or, if you installed the .desktop file, simply click the menu icon.
...
Make sure these endpoints are deployed and correctly configured.

🐞 Troubleshooting
"Unknown property transition" warnings – these are harmless QSS warnings; you can ignore them.

Login fails – check that your api_login.php is reachable and returns valid JSON.

GUI doesn't start – ensure all dependencies are installed (pip3 install -r requirements.txt).
